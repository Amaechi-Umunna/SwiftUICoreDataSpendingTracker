...
struct AddCardForm: View {
    ...
    var body: some View {
        NavigationView {
            //            let saveButton: Button<Text> = Button(action: {
            //                presentationMode.wrappedValue.dismiss()
            //            }, label: {
            //                Text("Save")
            //            })
            //            let cancelButton: Button<Text> = Button (action: { presentationMode.wrappedValue.dismiss()
            //            }, label: {
            //                Text("Cancel")
            //            })
            Form {
                Section(header: Text("Card Info")) {
                    ...
                    Picker("Type", selection: $cardType) {
                        ForEach(["Visa", "Mastercard", "Discover", "Citibank"], id: \.self) {
                            cardType in Text(String(cardType)).tag(String(cardType))
                        }
                    }
                }
                Section(header: Text("Expiration")) {
                    Picker("Month", selection: $month) {
                        ForEach(1..<13, id: \.self) {
                            num in Text(String(num)).tag(String(num))
                        }
                    }
